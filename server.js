...
function Static_Ressource_Request(req, res) {
    ...
    return staticRessourcesServer.sendRequestedFile(req, res); // check 'Content-Type
}
...
//require('./staticRessourcesServer.js') check 'Content-Type;
    if (!Static_Ressource_Request(req, res))
        if (!CORS_Prefligth(req, res))//if req.method==optional
